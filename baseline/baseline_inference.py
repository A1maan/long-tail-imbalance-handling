...
class BaselineInferenceRunner:
    ...
    def __init__(self, val_dir="/home/almaankhan/data/coco/images/val2017",
                 ...
                 model_dir="/home/almaankhan/model",
                 output_dir="runs/baseline_inference"):
        """
    ...
    def save_results(self):
        ...
        # 1. Save primary metrics (JSON)
        json_path = os.path.join(self.output_dir, "baseline_results_primary_metrics.json")
        with open(json_path, "w") as f:
            json.dump(self.results, f, indent=2)
        print(f"\n✓ Primary metrics saved to: {json_path}")
        
        # 2. Save primary metrics (CSV)
        csv_path = os.path.join(self.output_dir, "baseline_results_summary.csv")
        ...
        # 3. Save detailed class-wise metrics (JSON)
        detailed_path = os.path.join(self.output_dir, "baseline_results_class_wise.json")
        with open(detailed_path, "w") as f:
            json.dump(self.detailed_metrics, f, indent=2)
        print(f"✓ Class-wise metrics saved to: {detailed_path}")
        
        # 4. Create comprehensive comparison CSV
        comparison_path = os.path.join(self.output_dir, "baseline_results_comprehensive.csv")
        with open(comparison_path, "w", newline="") as f:
            fieldnames = [
                "Model", "Type", "mAP@0.5", "mAP@0.75", "mAP@[0.5:0.95]",
                "Precision", "Recall", "Num_Images", "Avg_Detections", "Inference_Time_ms", "Device"
            ]
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            
            for result in self.results:
                # Helper function to safely format None values
                ...
                writer.writerow({
                    ...
                    "Head_Classes_AP": safe_format(result.get('head_classes_AP'), fmt=".4f"),
                    "Medium_Classes_AP": safe_format(result.get('medium_classes_AP'), fmt=".4f"),
                    "Tail_Classes_AP": safe_format(result.get('tail_classes_AP'), fmt=".4f"),
        ...
        print(f"✓ Comprehensive metrics CSV saved to: {comparison_path}")
